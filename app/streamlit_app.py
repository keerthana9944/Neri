        ...
        <div style="margin-bottom: 40px;">
            ...
            <div style="background-color: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 14px; padding: 28px; box-shadow: 0 4px 20px -4px rgba(15, 23, 42, 0.06);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px;">
                    <div style="background-color: #F5F3FF; border-radius: 10px; padding: 16px; border-left: 4px solid #7C3AED;">
...
if not st.session_state["logged_in"]:
    ...
    with login_container:
        ...
        st.markdown(
            clean_html(f"""
            <div style="background: #F5F3FF; border: 1px solid #DDD6FE; border-radius: 8px; padding: 8px 12px; margin: 12px 0 16px 0; font-size: 0.775rem; color: #6D28D9; display: flex; align-items: center; justify-content: space-between;">
...
# Sidebar Navigation (Redesigned Enterprise Sidebar in #1E1035)
with st.sidebar:
...
if selected_nav == "Dashboard":
    ...
    st.markdown(
        ...
        <div class="neri-card-header">
            {get_svg_icon("clock", color="#1E1035", size=20)} Recent Activity
        </div>
        """),
        unsafe_allow_html=True
    )
                    ...
                    st.markdown(
                        ...
                        <div class="document-box">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div>
                                    <strong style="color: #1E1035;">{m_name}</strong> 
    ...
    if st.session_state.get("troubleshooting_result") is not None:
        ...
        if not is_reliable:
            ...
            if safety_warning:
                ...
                st.markdown(
                    clean_html(f"""
                    <div class="neri-card" style="border-left: 4px solid #7C3AED; background: #FFFFFF; padding: 24px;">
                        <div style="display: flex; align-items: center; gap: 10px; font-size: 1.1rem; font-weight: 700; color: #0F172A; margin-bottom: 8px;">
                            {get_svg_icon("document", color="#7C3AED", size=22)} No reliable answer found
                        </div>
                        <div style="font-size: 0.9rem; color: #64748B; line-height: 1.6; margin-bottom: 12px;">
                            {message_text}
                        </div>
                        <div style="font-size: 0.825rem; color: #7C3AED; background: #F5F3FF; padding: 10px; border-radius: 6px; font-weight: 500;">
                ...
                for doc_name, info in grouped_sources.items():
                    ...
                    <div class="source-box" style="margin-bottom: 10px;">
                        <div style="font-weight: 700; font-size: 0.9rem; color: #0F172A; margin-bottom: 6px; display: flex; align-items: center; justify-content: space-between;">
                            ...
                            <span style="font-size: 0.725rem; font-weight: 700; color: #7C3AED; background: #F5F3FF; padding: 2px 8px; border-radius: 4px;">{doc_type_str}</span>
                        </div>
